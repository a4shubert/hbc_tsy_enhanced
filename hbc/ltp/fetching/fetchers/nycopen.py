# file: hbc/ltp/fetching/fetchers/nycopen.py
...
NYC_BOROUGHS = {"BRONX", "BROOKLYN", "MANHATTAN", "QUEENS", "STATEN ISLAND", "UNSPECIFIED"}
...
class FetcherNYCOpenData(Fetcher):
    ...
    def validate(df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "unique_key" in df:
            ...
            mark(df["unique_key"].duplicated(keep="first"), "unique_key duplicate")
        try:
            mark(_to_hashable_df(df).duplicated(keep="first"), "full-row duplicate")
        except Exception as exc:
            # non-fatal diagnostic only
            reason_masks[f"duplicate check error: {exc!s}"] = pd.Series(False, index=df.index)
        ...
        if {"created_date", "closed_date"} <= set(df.columns):
            ...
            mark((xd.notna()) & (cd.notna()) & (xd < cd), "closed_date before created_date")

        if {"created_date", "resolution_action_updated_date"} <= set(df.columns):
            cd = _parse_dt(df["created_date"])
            rd = _parse_dt(df["resolution_action_updated_date"])
            mark((rd.notna()) & (cd.notna()) & (rd < cd), "resolution_action_updated_date before created_date")
        ...
        if "status" in df and "closed_date" in df:
            ...
            mark((~is_closed) & has_closed_dt, "closed_date set but status not closed")
            mark(is_closed & (~has_closed_dt), "closed status but closed_date missing")

        # --- Geography (advisory) ---
        if "latitude" in df:
            lat = pd.to_numeric(df["latitude"], errors="coerce")
            mark((lat < NYC_LAT_BOUNDS[0]) | (lat > NYC_LAT_BOUNDS[1]), "latitude outside NYC")
        if "longitude" in df:
            lon = pd.to_numeric(df["longitude"], errors="coerce")
            mark((lon < NYC_LON_BOUNDS[0]) | (lon > NYC_LON_BOUNDS[1]), "longitude outside NYC")
        ...
        if "x_coordinate_state_plane_" in df:
            ...
            mark((x < NYC_X_BOUNDS[0]) | (x > NYC_X_BOUNDS[1]), "x_coordinate_state_plane_ out of bounds")
        if "y_coordinate_state_plane_" in df:
            y = pd.to_numeric(df["y_coordinate_state_plane_"], errors="coerce")
            mark((y < NYC_Y_BOUNDS[0]) | (y > NYC_Y_BOUNDS[1]), "y_coordinate_state_plane_ out of bounds")
        ...
        if "incident_zip" in df:
            ...
            mark(mask_5 & ~z_num.between(ZIP_MIN, ZIP_MAX), "incident_zip outside NYC range")
        ...
        # # --- Required text (advisory) ---
        # for col in ["agency", "complaint_type", "descriptor", "location_type"]:
        #     if col in df:
        #         s = _nz(df[col])
        #         mark(s.eq(""), f"{col} blank")
        #         mark(s.str.len() > 200, f"{col} too long (>200)")

        # Create DROP_FLAG and DROP_REASON (no rows dropped)
        if reason_masks:
            masks_df = pd.DataFrame({k: v.reindex(df.index).fillna(False) for k, v in reason_masks.items()})
            drop_flag = masks_df.any(axis=1)
            drop_reason = masks_df.apply(lambda r: "; ".join(r.index[r.values]), axis=1)
        else:
        ...
        if total_flagged:
            counts = {reason: int(mask.sum()) for reason, mask in reason_masks.items()}
            parts = [f"{k}: {v}" for k, v in sorted(counts.items())]
            print(f"Validation summary -> flagged {total_flagged} rows. " + "; ".join(parts))
        else:
    ...
    def fetch(cls, config: Dict) -> pd.DataFrame:
        ...
        query_kwargs = {k: v for k, v in config.get("kwargs", {}).items() if v is not None}
        client = Socrata(base_url, app_token=token)

        if "limit" in query_kwargs:
            rows = client.get(dataset, **query_kwargs)  # respect caller's limit/offset
        else:
    ...
    def finalize(df: pd.DataFrame) -> pd.DataFrame:
        # No dropping; pass-through so you keep all rows with flags.
        return df
