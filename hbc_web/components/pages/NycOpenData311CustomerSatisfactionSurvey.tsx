...
export default function NycOpenData311CustomerSatisfactionSurvey({ docUrl }: { docUrl: string }) {
  ...
  const gridApiRef = useRef<GridApi<NycOpenData311CustomerSatisfactionSurvey> | null>(null)
  const [selectedCount, setSelectedCount] = useState(0)

  const columnDefs = useMemo<ColDef<NycOpenData311CustomerSatisfactionSurvey>[]>(
    () =>
      NYC_OPEN_DATA_311_CUSTOMER_SATISFACTION_SURVEY_COLUMNS.map((field) => ({
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      try {
        ...
        const parsed = parseNycOpenData311CustomerSatisfactionSurvey(payload)
