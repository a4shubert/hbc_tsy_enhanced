...
if (-not (Test-Path $VenvPy)) {
    Write-Host "[launch] .venv not found. Please run .\scripts\win\install.ps1 first."
    exit 1
}
...
Write-Host "[launch] Starting REST API in new window..."
...
Write-Host "[launch] Starting web app (prod) in new window..."
...
if ($Env:HBC_WEB_URL) {
    ...
    Write-Host "[launch] Opening browser at $Env:HBC_WEB_URL..."
    Start-Process $Env:HBC_WEB_URL | Out-Null
}

Write-Host "[launch] Starting demo notebook in new window..."
...
Write-Host "[launch] Three PowerShell windows started (REST API + web + notebook). This window remains free for job commands."
